...
import {
  Badge, BipolarBar, Empty, Kv, Meter, Note, PageHead, ProvenanceTag, Stat,
  fmt, pct, signed,
...
export default function DigitalTwin() {
  ...
  const model = twinState?.model
  const operating = twinState?.operating_point
  ...
  return (
    <>
      ...
      >
        ...
        <section className="panel">
          ...
          <div className="panel__body panel__body--tight">
            ...
            >
              <Repeat size={14} color="var(--residual)" />
              <span className="label" style={{ color: 'var(--residual)' }}>STATE FEEDBACK</span>
              <ArrowDown size={12} color="var(--ink-5)" />
              {/* The three quantities the estimator feeds back, named. */}
              <Badge tone="residual">VOLUMETRIC EFF</Badge>
              <Badge tone="residual">COOLING EFF</Badge>
              <Badge tone="residual">CYLINDER TRIM</Badge>
            </div>
          </div>
        </section>
      </div>
      ...
      <div className="grid" style={{ gridTemplateColumns: 'minmax(0, 1.35fr) minmax(0, 1fr)', marginBottom: 16 }}>
              ...
              <table className="table table--compact">
                ...
                <tbody>
                  {rows.map((row: any) => {
                    ...
                    return (
                      <tr key={row.channel}>
                        <td style={{ color: hot ? 'var(--crit)' : undefined }}>{row.label}</td>
                        <td className="num" style={{ color: 'var(--expected)' }}>{fmt(row.expected, 1)}</td>
                        <td className="num" style={{ color: 'var(--actual)' }}>{fmt(row.actual, 1)}</td>
                        <td className="num" style={{ color: 'var(--residual)' }}>{signed(row.residual, 2)}</td>
                        <td>
                          <BipolarBar value={row.normalised} range={6} />
                        </td>
                        <td className="num" style={{ color: hot ? 'var(--crit)' : 'var(--ink-4)' }}>
        ...
        <section className="panel">
          ...
          <div className="panel__body">
                ...
                <div key={item.k}>
                  <div className="row" style={{ justifyContent: 'space-between', marginBottom: 4 }}>
                    ...
                    <span className="mono" style={{ fontSize: 11 }}>
                      {fmt(item.current, 4)}
                      <span style={{ color: 'var(--ink-5)' }}> / {fmt(item.baseline, 4)} base</span>
                    </span>
                  </div>
            ...
            <span className="micro">PER-CYLINDER TRIM DIVERGENCE FROM HEALTHY BASELINE</span>
            <div className="stack stack--sm" style={{ marginTop: 8 }}>
              {(estimator?.trim_divergence ?? [0, 0, 0, 0]).map((value: number, i: number) => (
                ...
                >
                  <span
                    className="mono"
                    style={{ fontSize: 10.5, color: selected === i + 1 ? 'var(--accent)' : 'var(--ink-4)' }}
                  >
                    CYL {i + 1}
                  </span>
      ...
      <section className="panel" style={{ marginRight: 372 }}>
        <header className="panel__head">
          <h2 className="panel__title">Physics model</h2>
          <span className="panel__spacer" />
          <Badge tone={model?.physics_available ? 'ok' : 'caution'}>
            {model?.physics_backend ?? 'FGPISTON-CLASS'}
          </Badge>
        </header>
        <div className="panel__body">
          <div className="grid grid--3">
            <Kv
              items={[
                ['Backend', model?.physics_backend ?? '—'],
                ['Version', model?.physics_version ?? '—'],
            ...
            <Kv
              items={[
                ['Solver rate', `${fmt(model?.physics_steps_per_s, 0)} steps/s`],
                ['Real-time factor', `${fmt(model?.realtime_factor, 0)}×`],
                ['Operating point', `${fmt(operating?.rpm, 0)} rpm · ${fmt(operating?.map_inhg, 1)} inHg`],
                ['Power', `${fmt(operating?.power_hp, 1)} hp (${fmt(operating?.power_pct, 0)}%)`],
              ]}
            />
            <Kv
              items={[
                ['Altitude', `${(operating?.altitude_ft ?? 0).toLocaleString()} ft`],
                ['OAT', `${fmt(operating?.oat_c, 1)} °C`],
                ['IAS', `${fmt(operating?.ias_kt, 1)} kt`],
          ...
          <div style={{ marginTop: 14 }}>
            <div className="row row--tight row--wrap">
              <Badge tone="ok">VALIDATED THERMODYNAMIC</Badge>
              <Badge tone="caution">NOT CRANK-ANGLE CFD</Badge>
              <Badge tone="info">DIVERGENCE REPORTED</Badge>
            </div>
          </div>
        </div>
      </section>
