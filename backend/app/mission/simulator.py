...
def compare_profiles(
    ...
    """Rank candidate sorties by projected thermal stress on this engine."""
    out = []
    for profile in profiles:
        result = run_simulation(
            profile, engine_state, baseline_state, health_index, p_serviceable,
            run_id=f"CMP-{profile.id}", stride_s=90,
        )
        out.append({
            "profile_id": profile.id,
            "name": profile.name,
            "peak_cht_c": result.summary["peak_cht_c"],
            "cht_margin_c": result.summary["cht_margin_c"],
            "peak_asymmetry_c": result.summary["peak_asymmetry_c"],
            "risk_pct": result.summary["mission_risk"]["risk_pct"],
            "risk_band": result.summary["mission_risk"]["band"],
            "warnings": len(result.warnings),
        })
    out.sort(key=lambda r: r["risk_pct"])
    return out
...
def orbit_phase(t: float, period: float) -> float:
    return (t % period) / period * 2.0 * math.pi
