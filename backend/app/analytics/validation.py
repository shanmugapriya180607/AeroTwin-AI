...
def _calibration(confidences: list[float], correctness: list[int]) -> tuple[float, list[dict]]:
    """Retained for direct use; the fitted path goes through calibration.py."""
    return reliability(confidences, correctness, RELIABILITY_BINS)


def _unused_calibration(confidences: list[float], correctness: list[int]):
    bins: list[dict] = []
    ...
    for b in range(RELIABILITY_BINS):
        lo = b / RELIABILITY_BINS
        hi = (b + 1) / RELIABILITY_BINS
        members = [
            (c, k) for c, k in zip(confidences, correctness)
            if (c >= lo and c < hi) or (b == RELIABILITY_BINS - 1 and c == hi)
        ]
        if not members:
        ...
        observed = sum(k for _, k in members) / len(members)
